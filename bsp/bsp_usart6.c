...
void usart6_init(uint8_t *rx1_buf, uint8_t *rx2_buf, uint16_t dma_buf_num)
{
    ...
    //清除接收中断标志物位
    __HAL_DMA_CLEAR_FLAG(&hdma_usart6_rx, USART6_DMA_RX_ISR);
...
void usart6_tx_dma_enable(uint8_t *data, uint16_t len)
{
    ...
    //清除DMA发送中断标志物位
    __HAL_DMA_CLEAR_FLAG(&hdma_usart6_tx, USART6_DMA_TX_ISR);
...
{  
    if(USART6->ISR & UART_FLAG_IDLE)
	{
        ...
        //应用层定义回调函数
        usart6_receive_irq_callback();
    }
	else if(HAL_UART_GetError(&huart6) & HAL_UART_ERROR_NE){ /*!< Noise error             */
		//噪声错误
		__HAL_UART_CLEAR_NEFLAG(&huart6);
	}else if(HAL_UART_GetError(&huart6) & HAL_UART_ERROR_FE){ /*!< Frame error             */
		//帧格式错误
		__HAL_UART_CLEAR_FEFLAG(&huart6);
	}else if(HAL_UART_GetError(&huart6) & HAL_UART_ERROR_ORE){ /*!< Overrun error           */
		//数据太多串口来不及接收错误
		__HAL_UART_CLEAR_OREFLAG(&huart6);
	}
}
...
/**
  ...
  * @retval none
  */
__weak void usart6_receive_irq_callback(void)
{
