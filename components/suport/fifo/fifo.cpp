...
int fifo_single::fifo_s_get(uint8_t& element)
{
    int retval = FIFO_OK;
    //! add lock
...
int fifo_single::fifo_s_put(uint8_t element)
{
    int retval;
    //! add lock
    osMutexWait(fifo_mutexHandle, osWaitForever);
...
int fifo_single::fifo_s_puts(uint8_t* p_source, uint16_t len)
{
    int retval;
    uint16_t len_to_end;
    int len_from_start;
